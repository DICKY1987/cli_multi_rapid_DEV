...
class CLIArgs:
    ...
    # Tools & quality
    tools_cmd: Optional[str] = None
    quality_cmd: Optional[str] = None
...
def parse_args(argv: Optional[List[str]] = None) -> CLIArgs:
    ...
    quality_sub = quality_parser.add_subparsers(dest="quality_cmd", required=True)
    quality_run = quality_sub.add_parser("run", help="Run ruff/mypy/bandit/semgrep")
    quality_run.add_argument("--fix", dest="fix", action="store_true", help="Auto-fix ruff issues")

    # containers subcommand
    containers_parser = subparsers.add_parser("containers", help="Docker compose helpers")
    containers_sub = containers_parser.add_subparsers(dest="containers_cmd", required=True)
    containers_sub.add_parser("up", help="docker compose up -d using config/docker-compose.yml")
    containers_sub.add_parser("down", help="docker compose down using config/docker-compose.yml")
    ...
    return CLIArgs(
        ...
        tools_cmd=getattr(parsed, "tools_cmd", None),
        quality_cmd=getattr(parsed, "quality_cmd", None),
        fix=getattr(parsed, "fix", None),
    )
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    try:
        ...
        if args.command == "greet":
            ...
            tools_cmd = getattr(args, "tools_cmd", None)
            if tools_cmd == "doctor":
                try:
            ...
            quality_cmd = getattr(args, "quality_cmd", None)
            if quality_cmd == "run":
                try:
                    from integrations.process import ProcessRunner
                    from integrations.python_quality import QualitySuite
                    fix = bool(getattr(args, "fix", False))
                    runner = ProcessRunner(dry_run=False)
                    suite = QualitySuite(runner)
                    results = suite.run_all(fix=fix)
                    summary = {name: {"code": res.code, "duration_s": round(res.duration_s, 3)} for name, res in results.items()}
                    ...
                    print(f"Error: quality run failed: {exc}", file=sys.stderr)
                    return 1
            else:
                print("Error: unknown quality subcommand", file=sys.stderr)
                return 1
        elif args.command == "containers":
            cmd = getattr(args, "containers_cmd", None)
            compose_file = os.path.join("config", "docker-compose.yml")
            try:
                from integrations.process import ProcessRunner
                from integrations.containers import DockerAdapter
                runner = ProcessRunner(dry_run=False)
                docker = DockerAdapter(runner)
                if cmd == "up":
                    return docker.compose_up(compose_file)
                elif cmd == "down":
                    return docker.compose_down(compose_file)
                else:
                    print("Error: unknown containers subcommand", file=sys.stderr)
                    return 1
            except Exception as exc:
                print(f"Error: containers command failed: {exc}", file=sys.stderr)
                return 1
