...
for CLI Orchestrator operations.
"""
...
class RoleBasedAccessControl:
    ...
    def revoke_permissions_from_role(
        self, role: Role, permissions: list[Permission]
    ) -> None:
        """Revoke permissions from a role."""
        self._role_permissions[role].difference_update(permissions)

    def assign_permission_to_user(self, user_id: str, permission: Permission) -> None:
        """Assign additional permission directly to a user."""
        self._user_permissions[user_id].add(permission)

    def revoke_permission_from_user(self, user_id: str, permission: Permission) -> None:
    ...
    def get_user_permissions(self, user) -> Set[Permission]:
        ...
        # Direct permissions
        direct_permissions = self._user_permissions.get(user.id, set())
    ...
    def check_any_permission(self, user, permissions: list[Permission]) -> bool:
        """Check if user has any of the specified permissions."""
        user_permissions = self.get_user_permissions(user)
        return any(perm in user_permissions for perm in permissions)

    def check_all_permissions(self, user, permissions: list[Permission]) -> bool:
    ...
    def has_any_role(self, user, roles: list[Role]) -> bool:
    ...
    def get_effective_permissions_summary(self, user) -> Dict[str, any]:
        ...
        for role in user.roles:
            role_permissions[role.value] = [
                perm.value for perm in self._role_permissions.get(role, set())
            ]

        direct_permissions = [
            perm.value for perm in self._user_permissions.get(user.id, set())
        ]

        all_permissions = [perm.value for perm in self.get_user_permissions(user)]
