...
class JiraConnector:
        ...
        try:
            issue_data = {
                "fields": {
                    ...
                    "description": self._format_workflow_description(
                        workflow_id, workflow_data
                    ),
                    "issuetype": {"name": "Task"},
                    "priority": {
                        "name": self._get_priority_from_workflow(workflow_data)
                    },
            ...
            response = await self.session.post(
                f"{self.base_url}/rest/api/3/issue", json=issue_data
            )
            ...
            if response.status_code == 201:
                ...
                logger.info(
                    f"Created JIRA issue {issue_key} for workflow {workflow_id}"
                )
                ...
                logger.error(
                    f"Failed to create JIRA issue: HTTP {response.status_code}"
                )
                return None
    ...
    async def update_issue_progress(
        ...
        try:
            ...
            comment_data = {"body": self._format_progress_comment(progress_data)}
            ...
            if response.status_code == 201:
                ...
                logger.error(
                    f"Failed to update JIRA issue {issue_key}: HTTP {response.status_code}"
                )
                return False
    ...
    async def report_workflow_failure(
        ...
        try:
            ...
            update_data = {"fields": {"priority": {"name": "High"}}}

            await self.session.put(
                f"{self.base_url}/rest/api/3/issue/{issue_key}", json=update_data
            )
    ...
    def _format_workflow_description(
        ...
        if "estimated_duration" in workflow_data:
            description += (
                f"*Estimated Duration:* {workflow_data['estimated_duration']}\n"
            )
