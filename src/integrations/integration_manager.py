...
class IntegrationManager:
        ...
        for integration_type, config in self.configs.items():
            ...
            try:
                connector = await self._create_connector(
                    integration_type, config.config
                )
                if connector:
                    # Test connection
                    ...
                    if test_result.get("status") == "connected":
                        ...
                        logger.error(
                            f"Failed to connect to {integration_type.value}: {test_result.get('error')}"
                        )
    ...
    async def notify_error_recovery(
        self, error_code: str, recovery_action: str, success: bool
    ):
        ...
        if IntegrationType.SLACK in self.integrations:
            ...
            for channel in self.configs[IntegrationType.SLACK].channels:
                tasks.append(
                    slack.notify_error_recovery(
                        channel, error_code, recovery_action, success
                    )
                )
        ...
        if IntegrationType.TEAMS in self.integrations:
            ...
            tasks.append(
                teams.notify_error_recovery(error_code, recovery_action, success)
            )
    ...
    async def notify_cost_alert(self, service: str, cost_data: dict[str, Any]):
        ...
        if not self._check_rate_limit(
            f"cost_alert:{service}", 1800
        ):  # Max 1 every 30 minutes
            return
    ...
    async def _create_connector(
        ...
        if integration_type == IntegrationType.JIRA:
            ...
            return GitHubConnector(
                token=config["token"], organization=config.get("organization")
            )
    ...
    async def _create_workflow_resources(
        ...
        try:
            ...
            if IntegrationType.SLACK in self.integrations and workflow_data.get(
                "create_slack_channel"
            ):
                slack = self.integrations[IntegrationType.SLACK]
                channel_id = await slack.create_workflow_channel(
                    context.workflow_id, context.workflow_name
                )
                if channel_id:
    ...
    async def _create_jira_issue(
        ...
        try:
            ...
            issue = await jira.create_workflow_issue(
                context.workflow_id, workflow_data, context.project_key
            )
            if issue:
    ...
    async def _notify_slack_started(
        ...
        try:
            ...
            await slack.notify_workflow_started(
                context.slack_channel, context.workflow_id, workflow_data
            )
        except Exception as e:
    ...
    async def _notify_slack_progress(
        ...
        try:
            ...
            await slack.notify_workflow_progress(
                context.slack_channel, context.workflow_id, progress_data
            )
        except Exception as e:
    ...
    def _check_rate_limit(self, key: str, interval_seconds: int) -> bool:
        ...
        self.rate_limiters[key] = [
            timestamp for timestamp in self.rate_limiters[key] if timestamp > cutoff
        ]
