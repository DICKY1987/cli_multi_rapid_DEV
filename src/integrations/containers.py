...
from .tools_base import Containers, ToolProbe
...
class DockerAdapter:
    ...
    def __init__(self, runner: ProcessRunner) -> None:
        self.runner = runner
        self.binary = get_selected_tool_path("docker", "containers")
    ...
    def compose_up(
        ...
        args = [self.binary, "compose", "-f", compose_file, "up"]
        if detach:
            args.append("-d")
    ...
    def compose_down(self, compose_file: str = "docker-compose.yml") -> CommandResult:
        ...
        return self.runner.run([self.binary, "compose", "-f", compose_file, "down"])

    def ps(self) -> CommandResult:
        """List running containers."""
        return self.runner.run([self.binary, "ps"])
    ...
    def build(
        self, path: str = ".", tag: Optional[str] = None, dockerfile: str = "Dockerfile"
    ) -> CommandResult:
        """Build a Docker image."""
        args = [self.binary, "build", "-f", dockerfile]
        if tag:
            args.extend(["-t", tag])
        args.append(path)
        return self.runner.run(args)
    ...
    def pull(self, image: str) -> CommandResult:
        """Pull a Docker image."""
        return self.runner.run([self.binary, "pull", image])

    def push(self, image: str) -> CommandResult:
        """Push a Docker image."""
        return self.runner.run([self.binary, "push", image])

    def stop(self, container: str) -> CommandResult:
        """Stop a container."""
        return self.runner.run([self.binary, "stop", container])

    def rm(self, container: str, force: bool = False) -> CommandResult:
        """Remove a container."""
        args = [self.binary, "rm"]
        if force:
            args.append("-f")
    ...
    def info(self) -> CommandResult:
        ...
        return self.runner.run([self.binary, "info"])

    def images(self) -> CommandResult:
        """List Docker images."""
        return self.runner.run([self.binary, "images"])


def create_containers_adapter(
    runner: ProcessRunner, container_type: str = "docker"
) -> Containers:
    """Factory function to create containers adapter."""
    if container_type == "docker":
        return DockerAdapter(runner)
    else:
