"""Tool registry for detection, configuration, and selection."""
...
def load_config(config_path: str = "config/tool_adapters.yaml") -> ToolSelection:
    ...
    try:
        ...
        return ToolSelection(
            ...
            containers=data.get("containers", "docker"),
            editor=data.get("editor", "vscode"),
            js_runtime=data.get("js_runtime", "node"),
            ai_cli=data.get("ai_cli", "claude"),
            python_quality=data.get(
                "python_quality",
                {
                    "ruff": True,
                    "mypy": True,
            ...
            precommit=data.get("precommit", True),
...
def probe_binary(
    runner: ProcessRunner, binary: str, version_args: list[str]
) -> ToolProbe:
    """Probe a binary for version and availability."""
    try:
        res = runner.run([binary] + version_args)
...
def detect_all(runner: ProcessRunner) -> Dict[str, ToolProbe]:
    ...
    if sel.ai_cli == "openai":
        # Prefer direct binary when available
        if paths.get("openai"):
            probes["openai"] = probe_binary(runner, paths["openai"], ["--version"])
        else:
            # Try via npx
            probes["openai"] = probe_binary(
                runner, paths["npx"], ["openai", "--version"]
            )
    else:
...
def generate_doctor_report(probes: Dict[str, ToolProbe]) -> str:
    ...
    for name, probe in probes.items():
        ...
        version = f" (v{probe.version})" if probe.version else ""
        path = f" at {probe.path}" if probe.path else ""
        details = f" - {probe.details}" if probe.details else ""

        lines.append(f"{status} {name}{version}{path}{details}")

    # Summary
    ok_count = sum(1 for p in probes.values() if p.ok)
    total_count = len(probes)
    lines.extend(["", f"Summary: {ok_count}/{total_count} tools available"])

    return "\n".join(lines)


def get_selected_tool_path(tool_name: str, tool_type: str) -> str:
    """Get the configured path for a specific tool."""
    sel = load_config()

    # Check configured paths first
    if hasattr(sel, "paths") and sel.paths and tool_name in sel.paths:
        return sel.paths[tool_name]
    ...
    # Default to tool name (relies on PATH)
    return tool_name
