...
class TeamsConnector:
        ...
        try:
            if self.webhook_url:
                ...
                if response.status_code == 200:
                    ...
                    return {
                        "status": "failed",
                        "error": f"Webhook HTTP {response.status_code}",
                    }
            else:
    ...
    async def send_adaptive_card_v2(self, card_payload: dict[str, Any]) -> bool:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.error(
                    f"Failed to send adaptive card: HTTP {response.status_code}"
                )
                return False
            ...
            facts.append(
                {
                    ...
                    "value": workflow_data["estimated_duration"],
                }
            )

        if "tasks" in workflow_data:
            facts.append(
                {"name": "Total Tasks", "value": str(len(workflow_data["tasks"]))}
            )
    ...
    def _build_progress_card(
        ...
        facts = [{"name": "Progress", "value": f"{progress_bar} {progress_percent}%"}]

        if "current_phase" in progress_data:
            facts.append(
                {"name": "Current Phase", "value": progress_data["current_phase"]}
            )
            ...
            facts.append(
                {
                    ...
                    "value": str(progress_data["completed_tasks"]),
                }
            )
            ...
            facts.append(
                {
                    ...
                    "value": str(progress_data["remaining_tasks"]),
                }
            )
        ...
        facts.append(
            {"name": "Updated", "value": datetime.now().strftime("%Y-%m-%d %H:%M:%S")}
        )
            ...
            facts.append(
                {
                    ...
                    "value": str(result_data["tasks_completed"]),
                }
            )

        if "files_modified" in result_data:
            facts.append(
                {"name": "Files Modified", "value": str(result_data["files_modified"])}
            )
    ...
    def _build_failure_card(
        ...
        return TeamsCard(
            title="❌ Workflow Failed", text=text, color="Attention", facts=facts
        )

    def _get_theme_color(self, color: str) -> str:
        """Get theme color hex code."""
        color_map = {"Good": "00C851", "Warning": "FFB900", "Attention": "FF6900"}
