...
class SlackConnector:
    ...
    async def notify_error_recovery(
        self, channel: str, error_code: str, recovery_action: str, success: bool
    ) -> Optional[str]:
    ...
    async def notify_cost_alert(
        ...
        try:
            ...
            warning_level = (
                ":warning:" if usage_percent >= 80 else ":information_source:"
            )
    ...
    async def send_message(self, message: SlackMessage) -> Optional[str]:
        ...
        try:
            ...
            response = await self.session.post(
                f"{self.base_url}/chat.postMessage", json=payload
            )
    ...
    async def create_workflow_channel(
        ...
        try:
            ...
            response = await self.session.post(
                f"{self.base_url}/conversations.create", json=payload
            )
            ...
            if response.status_code == 200:
                ...
                if data.get("ok"):
                    ...
                    # Set channel topic
                    await self.set_channel_topic(
                        channel_id, f"Workflow: {workflow_name}"
                    )
    ...
    async def set_channel_topic(self, channel: str, topic: str) -> bool:
        ...
        try:
            ...
            response = await self.session.post(
                f"{self.base_url}/conversations.setTopic", json=payload
            )
    ...
    async def invite_users_to_channel(self, channel: str, users: list[str]) -> bool:
        ...
        try:
            ...
            response = await self.session.post(
                f"{self.base_url}/conversations.invite", json=payload
            )
    ...
    def _build_failure_blocks(
        ...
        if "recovery_attempted" in error_data:
            ...
            fields.append(
                {"type": "mrkdwn", "text": f"*Recovery Attempted:*\n{recovery_status}"}
            )
