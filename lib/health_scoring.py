        ...
        with self._lock:
            if self.state == CircuitState.CLOSED:
                ...
                if self.success_count >= self.success_threshold:
                    ...
                    self.failure_count = 0
                    logger.info(
                        f"Circuit breaker for {self.tool_name} closed (recovered)"
                    )
        ...
        with self._lock:
            ...
            if self.state == CircuitState.CLOSED:
                if self.failure_count >= self.failure_threshold:
                    ...
                    logger.warning(
                        f"Circuit breaker for {self.tool_name} opened (too many failures)"
                    )
            elif self.state == CircuitState.HALF_OPEN:
...
class HealthScoringSystem:
            ...
            with open(self.history_path) as f:
                for line in f:
                    if line.strip():
                        ...
                        # Limit size
                        if (
                            len(self.recent_metrics[tool_name])
                            > self.max_metrics_per_tool
                        ):
                            self.recent_metrics[tool_name] = self.recent_metrics[
                                tool_name
                            ][-self.max_metrics_per_tool :]
    ...
    def record_execution(
        ...
        if len(self.recent_metrics[tool_name]) > self.max_metrics_per_tool:
            self.recent_metrics[tool_name] = self.recent_metrics[tool_name][
                -self.max_metrics_per_tool :
            ]
        ...
        try:
            with open(self.history_path, "a") as f:
                history_entry = {"tool_name": tool_name, **asdict(metric)}
                f.write(json.dumps(history_entry) + "\\n")
        except Exception as e:
            logger.error(f"Failed to write health history: {e}")

    def calculate_health_score(
        self, tool_name: str, cost_hint: float = 0.0
    ) -> HealthScore:
        ...
        availability = len([m for m in recent_metrics if m.latency_ms > 0]) / len(
            recent_metrics
        )
        ...
        cost_efficiency_score = (
            max(0.0, 1.0 - (cost_hint / max_cost)) if cost_hint > 0 else 1.0
        )
    ...
    def get_system_health_summary(self) -> Dict[str, Any]:
        ...
        scores = self.get_all_health_scores()
        circuit_states = {
            tool_name: cb.get_state() for tool_name, cb in self.circuit_breakers.items()
        }
        ...
        cb_summary = {
            ...
            "open_circuits": len(
                [cb for cb in circuit_states.values() if cb["state"] == "open"]
            ),
            "half_open_circuits": len(
                [cb for cb in circuit_states.values() if cb["state"] == "half_open"]
            ),
            "closed_circuits": len(
                [cb for cb in circuit_states.values() if cb["state"] == "closed"]
            ),
        }
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Health scoring and circuit breaker management"
    )
    parser.add_argument(
        "command",
    ...
    parser.add_argument(
        "--success", action="store_true", help="Record successful execution"
    )
    ...
    if args.command == "record":
        if not args.tool or args.latency is None:
            print(
                "Error: --tool and --latency are required for record command",
                file=sys.stderr,
            )
            sys.exit(1)
        ...
        print(
            f"Recorded execution for {args.tool}: success={args.success}, latency={args.latency}ms"
        )
        ...
        if args.tool:
            ...
            print(
                json.dumps(
                    {name: asdict(score) for name, score in scores.items()}, indent=2
                )
            )
        ...
        for tool_name, cb in health_system.circuit_breakers.items():
            ...
            print(
                f"  {emoji} {tool_name}: {state['state']} (failures: {state['failure_count']})"
            )
