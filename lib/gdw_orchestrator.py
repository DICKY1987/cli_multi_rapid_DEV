...
def maybe_defer_to_gdw(
    task_description: str, repo_root: Path
) -> Optional[DeferDecision]:
        ...
        if needle and needle in task_description.lower():
            ...
            if wf:
                log_action(
                    ...
                    phase="route",
                    action="defer",
                    details={"workflow": wf, "reason": r.get("reason", "rule_match")},
                )
                return DeferDecision(
                    workflow_id=wf,
                    reason=str(r.get("reason", "rule_match")),
...
def defer_and_capture(
    decision: DeferDecision, repo_root: Path, dry_run: bool = False
) -> Dict[str, Any]:
