...
class ContextAnalysisEngine:
    ...
    async def analyze_project_context(
        ...
        project_context = ProjectContext(
            root_path=str(self.project_root), project_type="unknown"
        )
        ...
        try:
            ...
            audit_logger.log_action(
                ...
                "analysis",
                "error",
                {"error": str(e), "project_root": str(self.project_root)},
            )
    ...
    async def analyze_task_context(self, task_description: str) -> TaskContext:
        ...
        try:
            ...
            # Assess complexity
            task_context.complexity = await self._assess_task_complexity(
                task_description
            )

            # Determine priority
            task_context.priority = await self._determine_task_priority(
                task_description
            )

            # Identify affected files
            task_context.affected_files = await self._identify_affected_files(
                task_description
            )

            # Estimate effort
            task_context.estimated_effort_hours = await self._estimate_effort(
                task_context
            )
            ...
            audit_logger.log_action(
                ...
                "analysis",
                "error",
                {"error": str(e), "description": task_description},
            )
    ...
    async def suggest_workflows(
        ...
        template_names = template_matches.get(
            task_context.task_type, ["feature_development"]
        )
        ...
        for template_name in template_names:
            if template_name in self.workflow_templates:
                ...
                customized = await self._customize_workflow(
                    template, task_context, project_context
                )
        ...
        if task_context.complexity in [Complexity.COMPLEX, Complexity.CRITICAL]:
            ai_workflow = await self._create_ai_assisted_workflow(
    ...
    async def _customize_workflow(
        self,
        template: WorkflowSuggestion,
        task_context: TaskContext,
        project_context: ProjectContext,
    ) -> WorkflowSuggestion:
        ...
        customized.estimated_time_minutes = int(
            customized.estimated_time_minutes * multiplier
        )
    ...
    async def _create_ai_assisted_workflow(
        self, task_context: TaskContext, project_context: ProjectContext
    ) -> WorkflowSuggestion:
        ...
        return WorkflowSuggestion(
            ...
            estimated_time_minutes=int(
                task_context.estimated_effort_hours * 45
            ),  # 45 min per hour with AI assistance
...
async def main():
    ...
    task_context = await engine.analyze_task_context(
    ...
    for workflow in workflows:
        print(
            f"  - {workflow.name}: {workflow.estimated_time_minutes} min, {workflow.success_probability:.2%} success"
        )
