...
class AutomatedMergeStrategy:
    ...
    def __init__(self, config_path: Optional[str] = None):
        ...
        self.circuit_breakers = {
            tool.name: CircuitBreaker(
                tool.name, failure_threshold=0.3, recovery_timeout=300
            )
    ...
    async def _parse_merge_conflicts(self, merge_output: str) -> List[MergeConflict]:
        ...
        for match in conflict_pattern.finditer(merge_output):
            ...
            complexity = self._assess_conflict_complexity(
    ...
    def _assess_conflict_complexity(
        ...
        if (
            total_lines
            > self.strategy_rules["complexity_thresholds"]["complex_max_lines"]
        ):
            return ConflictComplexity.CRITICAL
        elif (
            total_lines
            > self.strategy_rules["complexity_thresholds"]["moderate_max_lines"]
            or complexity_score > 5
        ):
            return ConflictComplexity.COMPLEX
        elif (
            total_lines
            > self.strategy_rules["complexity_thresholds"]["simple_max_lines"]
            or complexity_score > 2
        ):
    ...
    async def select_optimal_merge_tool(
        ...
        max_complexity = max(
            (c.complexity for c in conflicts), default=ConflictComplexity.SIMPLE
        )
        total_cost_estimate = 0.0

        # Get prioritized tool list for this complexity
        candidate_tools = self.strategy_rules["tool_selection_priorities"][
            max_complexity
        ]
        ...
        for tool_name, tool in available_tools:
            ...
            if (
                ...
                and self.strategy_rules["cost_optimization"]["prefer_free_tools"]
            ):
                continue
    ...
    def _determine_merge_strategy(
        self, tool_name: str, complexity: ConflictComplexity
    ) -> MergeStrategy:
        ...
        elif tool.tool_type in [
            MergeToolType.CLAUDE_CODE,
            MergeToolType.GEMINI_CLI,
            MergeToolType.AIDER_LOCAL,
        ]:
    ...
    async def execute_merge(
        ...
        try:
            ...
            result = await self._execute_merge_strategy(
                tool_name, strategy, base_branch, feature_branch
            )

            # Record success
            self.health_tracker.record_success(
                tool_name, (datetime.utcnow() - start_time).total_seconds()
            )
            ...
            if fallback_tools:
                ...
                fallback_strategy = self._determine_merge_strategy(
                    fallback_tool,
                    max(
                        (c.complexity for c in conflicts),
                        default=ConflictComplexity.SIMPLE,
                    ),
                )
                return await self.execute_merge(
                    fallback_tool,
                    fallback_strategy,
                    conflicts,
                    base_branch,
    ...
    async def _ai_assisted_merge(
        self, tool_name: str, base_branch: str, feature_branch: str
    ) -> str:
...
async def main():
    ...
    # Execute merge
    result = await merge_strategy.execute_merge(
        tool, strategy, conflicts, "main", "feature-branch"
    )
