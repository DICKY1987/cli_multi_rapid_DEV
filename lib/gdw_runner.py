...
from .gdw_metrics import GDW_RUN_DURATION, GDW_RUNS_TOTAL
...
def run_gdw(
    ...
    if GDW_RUN_DURATION:
        try:
            GDW_RUN_DURATION.labels(workflow=spec.id).observe(
                max(0.0, time.time() - t0)
            )
        except Exception:
            pass
    publish_event(
        {"type": "gdw.end", "workflow": spec.id, "version": spec.version, "rc": rc}
    )
...
def execute_gdw(
    ...
    for step in spec.steps:
        ...
        rc = run_step(step, env=env, dry_run=dry_run)
        publish_event(
            {"type": "gdw.step", "workflow": spec.id, "step": step_id, "rc": rc}
        )
        record_gdw_cost(spec.id, step_id, amount=0.0)
        ...
        if rc != 0:
            break
    artifacts = (
        list(spec.observability.get("artifact_paths", []))
        if isinstance(spec.observability, dict)
        else []
    )
    return {
        "id": spec.id,
