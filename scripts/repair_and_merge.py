...
RE_CLAUDE = re.compile(r'"op"\s*:\s*"create_file"\s*,\s*"path"\s*:\s*"([^"]+)"\s*,\s*"content_base64"\s*:\s*"([^"]+)"', re.DOTALL)
RE_CODEX = re.compile(r'"operation"\s*:\s*"create"\s*,\s*"path"\s*:\s*"([^"]+)"\s*,\s*"content"\s*:\s*(\{[\s\S]*?\})\s*(?:\}|,|\])', re.DOTALL)
...
def extract_ops_from_text(text: str) -> List[Dict]:
    ...
    for m in RE_CLAUDE.finditer(text):
        ...
        ops.append({'op':'write_file','path':path,'content_base64':b64,'if_exists':'skip'})
    for m in RE_CODEX.finditer(text):
        ...
        try:
            ...
            continue
        ops.append({'op':'write_file','path':path,'content_base64':to_b64_json(obj),'if_exists':'skip'})
    return ops
...
def merge_into_plan(plan_path: Path, extracted_ops: List[Dict]) -> None:
    ...
    existing = set(op['path'] for op in plan.get('ops', []) if op.get('op')=='write_file')
        ...
        if 'file_operations' in obj:
            for it in obj['file_operations']:
                if it.get('operation') == 'create':
                    ops.append({'op':'write_file','path':it['path'],'content_base64':to_b64_json(it['content']),'if_exists':'skip'})
        if 'operations' in obj:
            for it in obj['operations']:
                if it.get('op') == 'create_file':
                    ops.append({'op':'write_file','path':it['path'],'content_base64':it['content_base64'],'if_exists':'skip'})
        if 'ops' in obj:
            for it in obj['ops']:
