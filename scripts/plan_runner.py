...
def op_assert_contains(op: dict, ctx: Context) -> None:\n    file_expr = op['file']\n    file_str = ctx.sub(file_expr)\n    if op.get('optional') and ('    file_path = Path(file_str)\n    if not file_path.exists():
...
if __name__ == '__main__':
 ...
 in file_str or -not (Test-Path -LiteralPath file_str)):\n        print(f'[assert] skip optional: {file_expr}')\n        return\n    file_path = Path(file_str)\n    if not file_path.exists():
        raise RuntimeError(f"assert_contains file not found: {file_path}")
    text = file_path.read_text(encoding='utf-8', errors='ignore')
    for token in op['must_contain']:
        if token not in text:
