...
OP_TYPES = {"write_file","replace_section","assert_contains","assert_not_contains","locate"}
...
def match_selector(op: dict, sel: dict) -> bool:
    ...
    if kind == 'regex':
        field = sel.get('field'); pattern = sel.get('pattern')
        val = op.get(field, '')
        try:
...
def remove_ops(module_name: str, selector: dict, max_remove: int|None, required: bool) -> int:
...
def add_module(filename: str, module_meta: dict|None, ops: list, position: str, relative_to: str|None):
    # Write module file
...
def apply_update(update_path: Path, schema_path: Path|None):
    ...
    for op in upd.get('operations', []):
        ...
        if t == 'add_module':
            ...
            add_module(op['filename'], op.get('module'), ops, op.get('position','end'), op.get('relative_to'))
            ...
            n = remove_ops(op['module'], op['selector'], op.get('max_remove'), op.get('required', True))
            ...
            pf = Path('paths.json')
            dump_json(pf, {'paths_allowlist': op.get('allow', []), 'paths_denylist': op.get('deny', [])})
...
def write_deliverables_manifest(update: dict):
    ...
    for op in update.get('operations', []):
        if op.get('type') in ('add_module','upsert_ops','replace_ops'):
            ...
            for w in ops:
                if (w.get('op') or w.get('type')) == 'write_file' and w.get('path') and w.get('content_base64'):
                    data = base64.b64decode(w['content_base64'])
                    created[w['path']] = hashlib.sha256(data).hexdigest()
    for d in deliverables:
        if d.get('type') == 'file' and not d.get('expected_sha256') and d.get('path') in created:
    ...
    (outdir / 'deliverables.manifest.json').write_text(json.dumps(manifest, indent=2), encoding='utf-8')
...
def main():
    ...
    ap.add_argument('--schema', default=str(Path('.ai/schemas/plan_update.schema.json')))
    args = ap.parse_args()
