#!/usr/bin/env python3
import argparse, base64, hashlib, json, sys, glob
...
def main():
    ...
    base = {
        'spec_version': '1.0',
        'metadata': {'name': 'combined_repo_plan', 'description': 'Composed plan from modules'},
        'defaults': {},
    ...
    if manifest.exists():
        ...
        extra = sorted([p.name for p in modules_dir.glob('*.json') if p.name not in existing and p.name != 'manifest.json'])
    ...
    ops_json = json.dumps(base['ops'], separators=(',', ':'), sort_keys=True).encode('utf-8')
    ...
    try:
        upd_manifests = sorted(glob.glob(str(Path('artifacts') / 'updates' / '*' / 'deliverables.manifest.json')))
        if upd_manifests:
            summaries = []
            for mf in upd_manifests:
                try:
                    m = load_json(Path(mf))
                    summaries.append({'update_id': m.get('update_id'), 'version': m.get('version'), 'deliverables': m.get('deliverables', [])})
                except Exception:
                    continue
            base.setdefault('metadata', {}).update({'deliverables_summary': summaries[-5:]})
    except Exception:
        pass
    ...
    if args.emit_plan_summary:
        ...
        summary = {
            ...
            'deliverables_summary': base.get('metadata', {}).get('deliverables_summary', [])
        }
        (summary_dir / 'plan.summary.json').write_text(json.dumps(summary, indent=2), encoding='utf-8')
