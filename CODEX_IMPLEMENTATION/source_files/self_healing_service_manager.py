...
class SelfHealingServiceManager:
        ...
        try:
            ...
            if graceful:
                ...
                try:
                    ...
                    logger.warning(
                        f"Graceful shutdown timeout for {service_name}, forcing"
                    )
    ...
    async def _check_service_health(self, service_name: str) -> None:
        ...
        if status.state == ServiceState.RUNNING:
            if not process or process.poll() is not None:
                ...
                logger.warning(
                    f"Service {service_name} died unexpectedly (exit code: {exit_code})"
                )
            ...
            if health_result:
                ...
                if service.restart_policy != RestartPolicy.NEVER:
                    await self.restart_service(
                        service_name, reason="health_check_failed"
                    )
        ...
        try:
            ...
            for name, status_data in state_data.get("services", {}).items():
                if name in self.service_status:
                    ...
                    status.last_restart = (
                        datetime.fromisoformat(status_data["last_restart"])
                        if status_data.get("last_restart")
                        else None
                    )
    ...
    def get_system_health_summary(self) -> Dict[str, Any]:
        ...
        running_services = len(
            [s for s in self.service_status.values() if s.state == ServiceState.RUNNING]
        )
        failed_services = len(
            [s for s in self.service_status.values() if s.state == ServiceState.FAILED]
        )
        ...
        return {
            ...
            "overall_health": (
                ...
                else "degraded" if running_services > 0 else "critical"
            ),
            "uptime_percentage": (
