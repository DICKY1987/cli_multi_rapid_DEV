...
from pty_terminal_runner import CommandRequest, MainWindow, TerminalTab
        ...
        with conn:
            ...
            if op == "start":
                ...
                if not cmd:
                    ...
                    state.tab.start_command(
                        CommandRequest(tool=cmd[0], args=cmd[1:], cwd=os.getcwd())
                    )
                    resp = {"ok": True}
                ...
                while time.time() - start < timeout:
                    ...
                    time.sleep(0.05)
                conn.sendall(
                    json.dumps({"ok": True, "exit": state.last_exit}).encode("utf-8")
                )
