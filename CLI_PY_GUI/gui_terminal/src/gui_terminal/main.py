...
def parse_arguments():
    ...
    parser.add_argument(
        "--security-policy", type=str, help="Path to security policy file"
    )
    parser.add_argument(
        "--no-security",
        action="store_true",
    ...
    parser.add_argument("--command", type=str, help="Command to execute on startup")
    parser.add_argument("--working-dir", type=str, help="Initial working directory")
...
def main():
    ...
    try:
        ...
        if not args.no_security:
            security_manager = SecurityPolicyManager(policy_path=args.security_policy)
