...
class StructuredLogger:
    ...
    def setup_handlers(self):
        ...
        if self.config.get("audit_logging", {}).get("enabled", True):
            audit_file = self.config.get("audit_logging", {}).get(
                "file", "logs/audit.log"
            )
    ...
    def security(self, violation_type: str, command: str, details: Dict[str, Any]):
        ...
        self.logger.warning(
            f"SECURITY: {violation_type}", extra={"audit_data": security_data}
        )
...
def setup_logging(log_level: str = "INFO"):
    ...
    logging_config = {
        "file_logging": {"enabled": True, "directory": "logs", "level": "INFO"},
        "console_logging": {"enabled": True, "level": log_level},
        "audit_logging": {"enabled": True, "file": "logs/audit.log"},
    }
    ...
    loggers = {
        ...
        "audit": {
            **logging_config,
            "console_logging": {"enabled": False, "level": "CRITICAL"},
        },
