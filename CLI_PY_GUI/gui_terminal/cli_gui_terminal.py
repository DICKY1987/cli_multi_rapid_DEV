...
class CLIMultiRapidGUI(QMainWindow):
    ...
    def create_toolbar(self):
        ...
        help_action.triggered.connect(
            lambda: self.execute_command("cli-multi-rapid --help")
        )
        toolbar.addAction(help_action)

        status_action = QAction("📊 Status", self)
        status_action.triggered.connect(
            lambda: self.execute_command("cli-multi-rapid workflow-status")
        )
        toolbar.addAction(status_action)

        streams_action = QAction("🔄 Streams", self)
        streams_action.triggered.connect(
            lambda: self.execute_command("cli-multi-rapid phase stream list")
        )
    ...
    def show_welcome_message(self):
        ...
        self.terminal.append_output(
            "🎯 Working Directory: " + str(self.working_directory), "yellow"
        )
        self.terminal.append_output("")
        self.terminal.append_output(
            "💡 Use the buttons on the left or type commands below", "cyan"
        )
        self.terminal.append_output("")
        self.show_prompt()
    ...
    def execute_command(self, command):
        ...
        if self.current_executor and self.current_executor.isRunning():
            self.terminal.append_output(
                "⚠️ Command already running. Please wait...", "yellow"
            )
            return
        ...
        self.current_executor.output_received.connect(
            lambda text: self.terminal.append_output(text, "white")
        )
        self.current_executor.error_received.connect(
            lambda text: self.terminal.append_output(text, "red")
        )
    ...
    def command_completed(self, return_code):
        ...
        if return_code == 0:
            ...
            self.terminal.append_output(
                f"❌ Command failed with return code: {return_code}", "red"
            )
