    ...
    def _run_in_gui(self, python_script: str) -> Dict[str, Any]:
        ...
        try:
            result = subprocess.run(
                [sys.executable, script_path], capture_output=True, text=True, timeout=self.timeout
            )
...
def main():
    ...
    parser.add_argument(
        "--gui-command",
        default="python gui/cli_terminal_gui.py",
        help="Command to launch GUI terminal",
    )
    parser.add_argument("--timeout", type=int, default=30, help="Timeout for individual tests")
    parser.add_argument(
        "--save-results", default="parity_test_results.json", help="File to save test results"
    )
