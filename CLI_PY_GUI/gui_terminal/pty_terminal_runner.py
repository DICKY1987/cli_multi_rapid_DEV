    ...
    def _run_windows(self):
        ...
        try:
            ...
            self.process = winpty.PtyProcess.spawn(
                cmdline, cwd=self.command_req.cwd, env=env
            )
            ...
            exit_code = (
                self.process.exitstatus if hasattr(self.process, "exitstatus") else 0
            )
            self.process_finished.emit(exit_code)
...
class TerminalTab(QWidget):
    ...
    def setup_ui(self):
        ...
        cmd_display.setStyleSheet(
            "font-family: monospace; background: #2d2d2d; padding: 4px; border-radius: 3px;"
        )
