...
class RuffSemgrepPlugin:
    ...
    def run(
        ...
        if config.get("run_semgrep", True):
            semgrep_result = self._run_semgrep(
                target_path, config.get("semgrep_config", {})
            )
    ...
    def _run_ruff(self, target_path: str, config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            message = (
                f"Ruff found {issues_count} issues"
                if issues_count > 0
    ...
    def _run_semgrep(self, target_path: str, config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if high_severity_count > 0:
                status = "FAIL"
                message = (
                    f"Semgrep found {high_severity_count} high-severity security issues"
                )
            elif len(findings) > 0:
                status = "WARNING"
