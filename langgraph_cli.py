    ...
    def select_optimal_agent(self, complexity: str) -> str:
        ...
        for service_name, config in SERVICES.items():
            ...
            if usage < config.daily_limit:
                ...
                print(
                    f"🎯 Selected: {service_name} (Usage: {usage}/{config.daily_limit})"
                )
    ...
    def build_graph(self) -> StateGraph:
        ...
        def analyze_task(state: AgentState) -> AgentState:
            ...
            return {
                ...
                "cost_estimate": SERVICES.get(
                    f"{selected_agent}_cli", SERVICES[f"{selected_agent}_local"]
                ).cost_per_request,
            }
...
def analyze(task_description: str):
    ...
    print(
        f"  Estimated Cost: ${SERVICES.get(f'{agent}_cli', SERVICES[f'{agent}_local']).cost_per_request:.2f}"
    )
